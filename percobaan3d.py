...
# Variabel untuk kontrol refleksi kincir angin
reflected = False  # Status refleksi awal
...
# Fungsi untuk menggambar kincir angin dengan refleksi
def draw_windmill(screen, windmill_image, x, y, reflected=False):
    if reflected:
        # Membalik gambar secara horizontal
        flipped_image = pygame.transform.flip(windmill_image, True, False)
        screen.blit(flipped_image, (x - flipped_image.get_width() // 2, y - flipped_image.get_height() // 2))
    else:
        # Gambar tanpa pembalikan
...
while running:
    ...
    # Perubahan refleksi kincir angin setiap 3 detik
    if pygame.time.get_ticks() % 3000 < 50:  # Modulo waktu untuk bergantian refleksi
    ...
    # Menggambar kincir angin dengan status refleksi
    draw_windmill(screen, windmill_image, windmill_rect.centerx, windmill_rect.centery, reflected)
